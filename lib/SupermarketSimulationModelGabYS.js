      ...
      switch (state) {
        case SHOPPING:
          if (Math.random() < probShopping) {
            ...
            /*             customer.target.row =
              selfCheckoutArea.startRow + Math.floor(queueOrder / 10);
            customer.target.col =
              selfCheckoutArea.startCol +
              selfCheckoutArea.numCols -
              1 -
              ((1 * queueOrder) % 10); */ //If we need a tolerance > 20, we pull this guy up
            customer.target.row =
              selfCheckoutArea.startRow + Math.floor(queueOrder / 10);
            customer.target.col =
              selfCheckoutArea.startCol +
