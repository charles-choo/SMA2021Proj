...
/* console.log(serviceCounters);
console.log(regularCheckout);
...
var areas = [
  {
    label: "Shopping Area",
    startRow: 2,
    numRows: 4 * numberOfRegularCounters,
    startCol: 2,
    numCols: 8,
    color: "pink",
  },
...
so there might be an error here too	*/
      ...
      if (Math.random() < probShopping) {
        ...
        nextCustomerID++;
        customerWaitingList.push(customer.id);
      }

      break;
    case WAITING:
      /* Hello Charles, we were thinking of adding some if-statements to the logic to control which counters
the customers move to based on their type (trolley vs basket).
We ran into an issue with the customers being stuck at the shopping area when we tried that	*/
      ...
      if (customer.type == "trolleyShopper") {
        ...
        if (
          ...
          customer.id == Math.min(...customerWaitingList)
        ) {
          ...
          for (var i = 0; i < customerWaitingList.length; i++) {
            if (customerWaitingList[i] == customer.id) {
              customerWaitingList.splice(i, 1);
            }
          }
        }
      } else {
        ...
        if (
          ...
          console.log(customerWaitingList);
        }
      }
    ...
    case TRANSACTION:
      ...
      if (hasArrived) {
        if (Math.random() < probDeparture) {
          ...
          } //here we added this to account for selfcheckout counters
