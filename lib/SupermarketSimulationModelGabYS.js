...
function range(start, end, step = 1) {
  ...
  var rowShelves = range(shoppingArea.startRow +1, shoppingArea.startRow+shoppingArea.numRows-2,4);
  var colShelves = range(shoppingArea.startCol+1, shoppingArea.startCol+shoppingArea.numCols-1, 2.5);
...
var regularCheckoutCustomerWaitingList = [];
...
function updateSurface() {
  ...
  newareas
	...
	.attr("height",  function(d){return 2.5*cellWidth;})
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for (i = 0; i < nextsteps.length-1; i++) {
        ...
        stepdistance[i]=Math.sqrt((nextrow-customer.target.row)*(nextrow-customer.target.row)+(nextcol-customer.target.col)*(nextcol-customer.target.col));
    } 
	...
	var nextstepisShelf=Shelves.filter(function(d){return d.row==nextsteprow && d.col==nextstepcol;});
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------      
	if (Math.random() < probShopping) {
        ...
        nextCustomerID++;
        if (customer.type == "trolleyShopper" && regularCheckoutCustomerWaitingList.length <= 2*queueToleranceValue) {
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------			  
    ...
    case WAITING:
      ...
      if (customer.type == "trolleyShopper") {
        var emptyCheckout = [];
        var emptyCheckout = regularCheckout.filter(function (counter) {
          return counter.state == IDLE;
        });

        if (
          emptyCheckout.length != 0 &&
          customer.id == Math.min(...regularCheckoutCustomerWaitingList)
        ) {
          //The customer will go to the first empty counter
          //console.log(emptyCheckout);

          var counterToServe = emptyCheckout[0];
          //console.log(counterToServe);
          customer.servedBy[0] = "Regular Checkout";
          customer.servedBy[1] = counterToServe.id;
          //We want to update the state of that counter that will serve the customer to be busy
          var index = regularCheckout
            .map((counter) => counter.id)
            .indexOf(counterToServe.id);
          regularCheckout[index].state = BUSY;

          //I want the customer to go to that counter
          customer.target.row =
            regularCounterStartRow + 2 * regularCheckout[index].id;
          customer.target.col = regularCounterStartCol;
          customer.state = TRANSACTION;

          for (var i = 0; i < regularCheckoutCustomerWaitingList.length; i++) {
            if (regularCheckoutCustomerWaitingList[i] == customer.id) {
              regularCheckoutCustomerWaitingList.splice(i, 1);
            }
          }
        }
      } else {
        ...
        console.log(selfCheckoutCustomerWaitingList.length);
