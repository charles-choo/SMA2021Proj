...
var probTechSavvyShopper = 0.3;
var queueToleranceValue = 28;
...
function updateSurface() {
  ...
  allstatistics.selectAll("text").text(function (d) {
    //var number = d.cumulativeValue / Math.max(1, d.count); // cumulativeValue and count for each statistic are always changing
    //return d.name + number;
    return "probArrival: " + probArrival;
          ...
          if (Math.random() < probShopping) {
            ...
            if (
              ...
              customer.target.row =
                ...
                Math.round(regularCheckoutArea.numRows / 2);
              customer.target.col =
                regularCheckoutArea.startCol +
                Math.round(regularCheckoutArea.numCols / 2);
        ...
        case WAITING:
          ...
          if (
            ...
            if (queueType == 1) {
              for (
                var i = 0;
                i < regularCheckoutCustomerWaitingList_1.length;
                i++
              ) {
                if (regularCheckoutCustomerWaitingList_1[i] == customer.id) {
                  regularCheckoutCustomerWaitingList_1.splice(i, 1);
                }
              }
            } else {
              for (
                var i = 0;
                i < regularCheckoutCustomerWaitingList_2.length;
                i++
              ) {
                if (regularCheckoutCustomerWaitingList_2[i] == customer.id) {
                  regularCheckoutCustomerWaitingList_2.splice(i, 1);
                }
              }
            }
          } else {
        ...
        case TRANSACTION:
          if (hasArrived) {
            ...
            if (Math.random() < probDeparture) {
              ...
              customer.target.col = maxCols - 13;
      ...
      switch (state) {
        case SHOPPING:
          ...
          if (Math.random() < probShopping) {
            ...
            customer.shoppingTime = currentTime - customer.timeAdmitted;
            //Over here, we will partition the checkout waiting area into 4 rows.
            //Customers will fill the first row (10), then the 2nd (20), then move to scan and go or what not
            queueOrder = selfCheckoutCustomerWaitingList.indexOf(customer.id);

            customer.target.row =
              selfCheckoutArea.startRow +
              Math.round(selfCheckoutArea.numRows / 2);
            customer.target.col =
              selfCheckoutArea.startCol +
              Math.round(selfCheckoutArea.numCols / 2);
          }
        ...
        case WAITING:
          ...
          //Case when there are no available checkout AND there are many people in the queue. Person will go scan and go
            ...
            selfCheckoutCustomerWaitingList.length > queueToleranceValue
          ) {
            ...
            customer.target.row =
              selfCheckoutArea.startRow +
              +queueOrder -
              (queueOrder - 7) * 2 * (queueOrder >= 7) +
              (queueOrder - 14) * 2 * (queueOrder >= 14) -
              (queueOrder - 21) * 2 * (queueOrder >= 21);
            customer.target.col =
              selfCheckoutArea.startCol +
              selfCheckoutArea.numCols -
              1 -
              Math.floor(queueOrder / 8);
          }
          break;
