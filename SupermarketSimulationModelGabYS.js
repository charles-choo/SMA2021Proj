...
function redrawWindow() {
  ...
  nextRegularServicedCustomerID = 1; //this is the id of the next customer of type B to be treated by the regularCheckout
  nextSelfCheckoutServicedCustomerID = 1;
...
We ran into an issue with the customers being stuck at the shopping area when we tried that	*/
      ...
      if (emptyselfCheckout.length != 0) {
        ...
        //I want the customer to go to that counter
        customer.target.row =
          selfCounterStartRow + 2 * selfCheckout[indexself].id;
        customer.target.col = selfCounterStartCol;
        customer.state = TRANSACTION;
      }
    ...
    case TRANSACTION:
      ...
      if (hasArrived) {
        if (Math.random() < probDeparture) {
		  ...
		  selfCheckout[customer.servedBy[1]].state = IDLE; //here we added this to account for selfcheckout counters
